...
class PgVectorStoreIntegrationTest {
    ...
    // ==================== Real Policy Document Tests ====================
    // These tests use content from the actual rebooking_policy.md document
    // to verify text search works with realistic domain content.

    /**
     * Insert chunks derived from the rebooking_policy.md document.
     * These chunks represent realistic content that would be generated by chunking the policy.
     */
    private fun insertPolicyChunks() {
        val policyChunks = listOf(
            // Gold Concierge Desk - this should match "gold service number"
            Triple(
                "policy-gold-concierge",
                """Gold Concierge Desk - Phone: 1-800-EMB-GOLD (1-800-362-4653) - International: +1-555-362-4653 - Available 24/7/365. Access to the Gold Concierge Desk, a dedicated 24/7 service line staffed by senior agents with enhanced authorization to process complex changes and approve exceptions.""",
                """{"section": "contact", "tier": "gold"}"""
            ),
            // Gold Status Benefits
            Triple(
                "policy-gold-benefits",
                """Gold Status Benefits Relevant to Rebooking: Members holding Gold Status shall receive the following comprehensive benefits applicable to this Policy: (a) 50% reduction in Base Change Fees (the "Gold Tier Multiplier"); (b) Maximum Extended Change Windows as specified in Section 13.2.4, including the ability to make changes up to 7 days in advance.""",
                """{"section": "benefits", "tier": "gold"}"""
            ),
            // Flight rebooking general
            Triple(
                "policy-rebooking-overview",
                """This Comprehensive Flight Rebooking, Modification, and Change Policy establishes the complete framework governing all requests by passengers to modify, alter, amend, reschedule, reroute, or otherwise change any aspect of a confirmed reservation for air transportation services operated by Embabel Air Corporation.""",
                """{"section": "overview"}"""
            ),
            // Change fee structure
            Triple(
                "policy-change-fees",
                """Base Change Fee Structure: The following base change fees apply to voluntary changes made by passengers. Fees vary by route type, timing of change, and membership tier. Domestic Short-Haul: $100 base fee. Domestic Long-Haul: $150 base fee. International: $200-$500 depending on destination.""",
                """{"section": "fees"}"""
            ),
            // Processing channels
            Triple(
                "policy-channels",
                """Processing Channels and Service Numbers: Reservations Phone available 24/7 for standard processing. Airport Ticket Counter during airport hours for expedited service. Gold Concierge Desk available 24/7 for priority Gold member service. Social Media and Chat support during extended hours.""",
                """{"section": "contact"}"""
            ),
            // Tier multipliers
            Triple(
                "policy-tier-multipliers",
                """Tier-Based Fee Modifications and Multipliers: No Status has a multiplier of 1.00 with 0% reduction. Bronze tier has a multiplier of 0.85 with 15% reduction. Silver tier has a multiplier of 0.70 with 30% reduction. Gold tier has a multiplier of 0.50 with 50% reduction in fees.""",
                """{"section": "fees", "tier": "all"}"""
            ),
            // Appeal process
            Triple(
                "policy-appeals",
                """Appeals and Exceptions Process: Level 4 Executive Committee Review is available for Gold Members Only. Filing Method requires Gold Concierge Desk submission. Prerequisite is Level 3 decision received and Gold status membership.""",
                """{"section": "appeals", "tier": "gold"}"""
            ),
            // Contact information
            Triple(
                "policy-contact-info",
                """Customer Relations Contact Information - Email: customerrelations@embabelair.com. Gold Concierge Desk Phone: 1-800-EMB-GOLD (1-800-362-4653). International contact number: +1-555-362-4653. Service available 24 hours a day, 7 days a week, 365 days a year.""",
                """{"section": "contact", "tier": "gold"}"""
            )
        )

        policyChunks.forEach { (id, text, metadata) ->
            jdbcClient.sql("""
                INSERT INTO content_elements (id, text, urtext, parent_id, labels, metadata)
                VALUES (:id, :text, :text, 'policy-doc', ARRAY['Chunk']::text[], :metadata::jsonb)
            """.trimIndent())
                .param("id", id)
                .param("text", text)
                .param("metadata", metadata)
    ...
    @Test
    fun `policy search - gold service number should match Gold Concierge Desk content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        val request = TextSimilaritySearchRequest(
            query = "gold service number",
            topK = 3,
            similarityThreshold = 0.0  // First verify we get results at all
        )
        val results = store.textSearch(request, Chunk::class.java)

        assertTrue(results.isNotEmpty(), "Should find results for 'gold service number'")

        // Print results for debugging
        println("Results for 'gold service number':")
        results.forEach { println("  - ${it.match.id}: score=${it.score}") }

        // Should find Gold Concierge Desk content
        val matchingIds = results.map { it.match.id }
        assertTrue(
            matchingIds.any { it.contains("gold") || it.contains("contact") },
            "Should find Gold-related or contact chunks"
        )
    }

    @Test
    fun `policy search - gold service number with 0_7 threshold should return results`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        // This is the actual query that was failing in downstream projects
        val request = TextSimilaritySearchRequest(
            query = "gold service number",
            topK = 3,
    ...
    @Test
    fun `policy search - rebooking policy should find overview content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()
        ...
        assertTrue(results.isNotEmpty(), "Should find results for 'rebooking policy'")
        assertTrue(
            results.any { it.match.id == "policy-rebooking-overview" },
            "Should find the rebooking overview chunk"
        )
    }

    @Test
    fun `policy search - change fees should find fee structure content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()
        ...
        assertTrue(results.isNotEmpty(), "Should find results for 'change fees'")

        val feeChunkFound = results.any {
            it.match.id == "policy-change-fees" || it.match.id == "policy-tier-multipliers"
        }
        assertTrue(feeChunkFound, "Should find fee-related chunks")
    }

    @Test
    fun `policy search - flight modification should find relevant content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        val request = TextSimilaritySearchRequest(
            query = "flight modification",
            topK = 5,
            similarityThreshold = 0.0
        )
        val results = store.textSearch(request, Chunk::class.java)

        assertTrue(results.isNotEmpty(), "Should find results for 'flight modification'")
    }

    @Test
    fun `policy search - gold member benefits should find benefits content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        val request = TextSimilaritySearchRequest(
            query = "gold member benefits",
            topK = 5,
            similarityThreshold = 0.0
        )
        val results = store.textSearch(request, Chunk::class.java)

        assertTrue(results.isNotEmpty(), "Should find results for 'gold member benefits'")
        assertTrue(
            results.any { it.match.id == "policy-gold-benefits" },
            "Should find the gold benefits chunk"
        )
    }

    @Test
    fun `policy search - appeal process gold should find appeals content`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        val request = TextSimilaritySearchRequest(
            query = "appeal process gold",
            topK = 5,
            similarityThreshold = 0.0
        )
        val results = store.textSearch(request, Chunk::class.java)

        assertTrue(results.isNotEmpty(), "Should find results for 'appeal process gold'")
        assertTrue(
            results.any { it.match.id == "policy-appeals" },
            "Should find the appeals chunk"
        )
    }

    @Test
    fun `policy search - verify scores are properly normalized for threshold filtering`() {
        val store = createStore()
        store.provision()
        insertPolicyChunks()

        // Query that should have strong matches
        val request = TextSimilaritySearchRequest(
            query = "Gold Concierge Desk phone number",
            topK = 10,
            similarityThreshold = 0.0
        )
        val results = store.textSearch(request, Chunk::class.java)
        ...
        println("Score distribution for 'Gold Concierge Desk phone number':")
        results.forEach { result ->
            println("  - ${result.match.id}: score=${result.score}")
        }

        // Verify all scores are normalized 0-1
        results.forEach { result ->
            assertTrue(result.score >= 0.0 && result.score <= 1.0,
                "Score should be normalized: ${result.score}")
        }

        // The best match should have a reasonably high score
