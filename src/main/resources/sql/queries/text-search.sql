...
-- Uses websearch_to_tsquery for web-like search syntax (OR, -, "phrases")
...
SELECT id, uri, text, urtext, parent_id, labels, metadata, ingestion_timestamp,
       ts_rank(tsv, websearch_to_tsquery('english', :query)) as raw_score,
       LEAST(1.0, ts_rank(tsv, websearch_to_tsquery('english', :query)) * 3) as score
FROM {table}
